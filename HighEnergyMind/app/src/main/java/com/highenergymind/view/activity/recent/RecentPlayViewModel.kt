...
class RecentPlayViewModel @Inject constructor(
    ...
    val getRecentResponse by lazy { MutableSharedFlow<ResponseResult<ResponseWrapper>>() }
    ...
    fun getRecentApi() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val response =
                getResult(
                    { apiService.recentTrackList(map) },
                    ApiEndPoint.RECENT_TRACK_LIST
                )
            getRecentResponse.emit(response)
