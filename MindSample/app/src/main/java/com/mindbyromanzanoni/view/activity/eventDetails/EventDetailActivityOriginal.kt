...
import com.mindbyromanzanoni.databinding.ActivityEventDetailOriginalBinding
...
@AndroidEntryPoint
class EventDetailActivityOriginal : BaseActivity<ActivityEventDetailOriginalBinding>() {
    private val viewModal: HomeViewModel by viewModels()
    var activity = this@EventDetailActivityOriginal
    ...
    override fun getLayoutRes(): Int {
        return R.layout.activity_event_detail_original
    }
    ...
    private fun clickListeners() {
        binding.apply {
            ...
            tvViewAllComments.setOnClickListener {
                ...
                bottomSheetComments.dismissCallBackSheet = { status, count ->
                    if (status) {
                        ...
                        tvViewAllComments.text = getString(R.string.view_all).plus(
                            count.toString().plus(" ").plus("comments")
                        )
            ...
            layout1.setOnClickListener {
                startZoomMeeting(this@EventDetailActivityOriginal, "2324")
            }
            ...
            icZoom.setOnClickListener {
                if (isVideo(eventDetails?.mediaPath)) {
    ...
    private fun setDataApi(data: Details) {
        binding.apply {
            ...
            tvViewAllComments.text = getString(R.string.view_all).plus(
                data.totalComments.toString().plus(" ").plus("comments")
            )
            likeUser()
    ...
    private fun initPlayer(mediaPath: String) {
        player = ExoPlayer.Builder(applicationContext).build().apply { addListener(playerListener) }
        setUri(mediaPath)
