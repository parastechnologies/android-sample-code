...
import com.mindbyromanzanoni.data.response.ChatCountResponseModel
import com.mindbyromanzanoni.data.response.CommonResponse
import com.mindbyromanzanoni.data.response.allCategoryDetail.AllTypeCategoryDetail
...
class HomeViewModel @Inject constructor(private val authDataSourceImp: HomeDataSourceImp) :
    ...
    private val _allTypeResponse: MutableSharedFlow<Resource<AllTypeCategoryDetail>> =
        MutableSharedFlow()
    val allTypeResponse = _allTypeResponse.asSharedFlow()

    private val changePasswordResponse: MutableSharedFlow<Resource<CommonResponse>> =
        MutableSharedFlow()
    val changePasswordSharedFlow = changePasswordResponse.asSharedFlow()

    private val meditationTypeListResponse: MutableSharedFlow<Resource<MeditationTypeResponse>> =
        MutableSharedFlow()
    val meditationTypeListSharedFlow = meditationTypeListResponse.asSharedFlow()

    private val meditationCatListResponse: MutableSharedFlow<Resource<MeditationCatResponse>> =
        MutableSharedFlow()
    val meditationCatListSharedFlow = meditationCatListResponse.asSharedFlow()

    private val searchCatOrSubCatResponse: MutableSharedFlow<Resource<SearchCatOrSubCatResponse>> =
        MutableSharedFlow()
    val searchCatOrSubCatSharedFlow = searchCatOrSubCatResponse.asSharedFlow()

    private val edificationListResponse: MutableSharedFlow<Resource<EdificationTypeResponse>> =
        MutableSharedFlow()
    val edificationListSharedFlow = edificationListResponse.asSharedFlow()

    private val edificationCategoryListResponse: MutableSharedFlow<Resource<EdificationCatResponse>> =
        MutableSharedFlow()
    val edificationCategoryListSharedFlow = edificationCategoryListResponse.asSharedFlow()

    private val journalListResponse: MutableSharedFlow<Resource<JournalResponse>> =
        MutableSharedFlow()
    val journalListSharedFlow = journalListResponse.asSharedFlow()

    private val notificationListResponse: MutableSharedFlow<Resource<NotificationResponse>> =
        MutableSharedFlow()
    val notificationListSharedFlow = notificationListResponse.asSharedFlow()

    private val addJournalResponse: MutableSharedFlow<Resource<CommonResponse>> =
        MutableSharedFlow()
    val addJournalSharedFlow = addJournalResponse.asSharedFlow()

    private val resourceListResponse: MutableSharedFlow<Resource<ResourceCategoryResponse>> =
        MutableSharedFlow()
    ...
    private val searchMeditationResponse: MutableSharedFlow<Resource<MeditationCatResponse>> =
        MutableSharedFlow()
    val searchMeditationSharedFlow = searchMeditationResponse.asSharedFlow()

    private val updateNotificationSettingResponse: MutableSharedFlow<Resource<CommonResponse>> =
        MutableSharedFlow()
    val updateNotificationSettingSharedFlow = updateNotificationSettingResponse.asSharedFlow()

    private val commentListResponse: MutableSharedFlow<Resource<CommentResponse>> =
        MutableSharedFlow()
    ...
    private val eventDetailsResponse: MutableSharedFlow<Resource<EventDetailsResponse>> =
        MutableSharedFlow()
    val eventDetailsSharedFlow = eventDetailsResponse.asSharedFlow()

    private val updateJournalResponse: MutableSharedFlow<Resource<CommonResponse>> =
        MutableSharedFlow()
    val updateJournalSharedFlow = updateJournalResponse.asSharedFlow()

    private val addCommentResponse: MutableSharedFlow<Resource<AddCommentResponse>> =
        MutableSharedFlow()
    val addCommentSharedFlow = addCommentResponse.asSharedFlow()

    private val favouriteStatusEventResponse: MutableSharedFlow<Resource<CommonResponse>> =
        MutableSharedFlow()
    val favouriteEventStatusSharedFlow = favouriteStatusEventResponse.asSharedFlow()

    private val userProfileResponse: MutableSharedFlow<Resource<UserDataResponse>> =
        MutableSharedFlow()
    ...
    private val journalDetailResponse: MutableSharedFlow<Resource<ViewJournalsResponse>> =
        MutableSharedFlow()
    val journalDetailSharedFlow = journalDetailResponse.asSharedFlow()

    private val resourceListByTypeResponse: MutableSharedFlow<Resource<ResourceTypeResponse>> =
        MutableSharedFlow()
    val resourceListByTypeSharedFlow = resourceListByTypeResponse.asSharedFlow()

    private val deleteJournalResponse: MutableSharedFlow<Resource<CommonResponse>> =
        MutableSharedFlow()
    val deleteJournalSharedFlow = deleteJournalResponse.asSharedFlow()

    private val deleteAccountResponse: MutableSharedFlow<Resource<CommonResponse>> =
        MutableSharedFlow()
    val deleteAccountSharedFlow = deleteAccountResponse.asSharedFlow()

    private val notificationStatusResponse: MutableSharedFlow<Resource<NotificationStatusResponse>> =
        MutableSharedFlow()
    val notificationStatusSharedFlow = notificationStatusResponse.asSharedFlow()


    private val chatCountResponse: MutableSharedFlow<Resource<ChatCountResponseModel>> =
        MutableSharedFlow()
    val chatCount = chatCountResponse.asSharedFlow()

    private val chatUsersResponse: MutableSharedFlow<Resource<ChatUsersResponse>> =
        MutableSharedFlow()
    ...
    private val notificationReminderResponse: MutableSharedFlow<Resource<CommonResponse>> =
        MutableSharedFlow()
    ...
    suspend fun responseOnTheBasesOfCategory(eventId: String, type: String) {
        showLoading.postValue(true)
        val param = HashMap<String, String>()
        param[ApiConstants.ApiParams.TYPE.value] = type
        param[ApiConstants.ApiParams.ID.value] = eventId
        authDataSourceImp.executeAllTypeDetail(param, apiType = ApiConstants.ALL_TYPE_DETAIL)
            .catch { e ->
                _allTypeResponse.emit(Resource.Error(e.message.toString()))
                showLoading.postValue(false)
            }.collect { isResponse ->
                _allTypeResponse.emit(isResponse)
                showLoading.postValue(false)
    ...
    suspend fun hitSearchApiAccordingToCatOrSubCatApi() {
        ...
        authDataSourceImp.executeSearchCatOrSubCatApi(
            param,
            apiType = ApiConstants.SEARCH_API_DASHBOARD
        )
            .catch { e ->
    ...
    suspend fun hitAddJournalApi() {
        ...
        authDataSourceImp.executeAddJournalApi(
            jsonObjectAddJournalWeight.toString().createPartFromJsonString(),
            apiType = ApiConstants.ADD_JOURNAL
        )
            .catch { e ->
    ...
    suspend fun hitResourceCategoryApi() {
        ...
        authDataSourceImp.executeResourcesCategoryApi(apiType = ApiConstants.RESOURCE_CATEGORY)
            .catch { e ->
    ...
    suspend fun hitUpdateJournalApi() {
        showLoading.postValue(true)
        authDataSourceImp.executeUpdateJournalApi(
            jsonObjectAddJournalWeight.toString().createPartFromJsonString(),
            apiType = ApiConstants.UPDATE_JOURNAL
        )
            .catch { e ->
    ...
    suspend fun hitNotificationReminderApi() {
        ...
        param[ApiConstants.ApiParams.REMINDER_DATE.value] =
            notificationReminderDate.get()?.trim() ?: ""
        authDataSourceImp.executeNotificationReminderApi(
            param,
            apiType = ApiConstants.NOTIFICATION_ADD_REMINDER
        )
            .catch { e ->
    ...
    suspend fun hitUpdateFavouriteEventStatusApi() {
        ...
        param[ApiConstants.ApiParams.TYPE.value] = type.get() ?: 4
    ...
    suspend fun hitGetJournalDetailApi() {
        ...
        authDataSourceImp.executeGetJournalDetailApi(
            param,
            apiType = ApiConstants.GET_JOURNAL_DETAIL
        ).catch { e ->
    ...
    suspend fun hitResourceListByTypeApi() {
        ...
        authDataSourceImp.executeResourceListByTypeApi(
            param,
            apiType = ApiConstants.RESOURCE_LIST_BY_TYPE
        ).catch { e ->
    ...
    suspend fun hitDeleteJournalApi() {
        ...
        val param = HashMap<String, String?>()
        param[ApiConstants.ApiParams.JOURNAL_ID.value] = journalID.get()?.trim() ?: ""
        authDataSourceImp.executeDeleteJournalApi(param, apiType = ApiConstants.DELETE_JOURNAL)
            .catch { e ->
     ...
     * this function is use to hit Delete Journal Api
     * */
    suspend fun hitDeleteUserApi(hashmap: HashMap<String, String>) {
        showLoading.postValue(true)
        authDataSourceImp.executeDeleteUserApi(hashmap,apiType = ApiConstants.DELETE_ACCOUNT)
            .catch { e ->
                deleteAccountResponse.emit(Resource.Error(e.message.toString()))
                showLoading.postValue(false)
            }.collect { isResponse ->
                showLoading.postValue(false)
                deleteAccountResponse.emit(isResponse)
            }
    }
    /**
    ...
    suspend fun hitNotificationStatusApi() {
        authDataSourceImp.executeNotificationStatusApi(apiType = ApiConstants.NOTIFICATION_STATUS)
            .catch { e ->
    ...
    /**
     * this function is use to get chat count api
     * */
    suspend fun hitChatCountApi() {
        authDataSourceImp.executeChatCountApi(apiType = ApiConstants.CHAT_COUNT)
            .catch { e ->
                chatCountResponse.emit(Resource.Error(e.message.toString()))
            }.collect { isResponse ->
                chatCountResponse.emit(isResponse)
            }
    }
    /**
