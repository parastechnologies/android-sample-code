package com.highenergymind.view.fragment.favoritePager

import android.content.Context
import androidx.lifecycle.viewModelScope
import com.highenergymind.api.ApiConstant
import com.highenergymind.api.ApiEndPoint
import com.highenergymind.api.ApiService
import com.highenergymind.api.ResponseResult
import com.highenergymind.api.ResponseWrapper
import com.highenergymind.api.getResult
import com.highenergymind.base.BaseViewModel
import com.highenergymind.utils.AppConstant
import dagger.hilt.android.lifecycle.HiltViewModel
import dagger.hilt.android.qualifiers.ApplicationContext
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.flow.MutableSharedFlow
import kotlinx.coroutines.launch
import javax.inject.Inject


/**
 * Created by developer on 29/03/24
 */
@HiltViewModel
class FavViewModel @Inject constructor(
    @ApplicationContext context: Context,
    val apiService: ApiService
) : BaseViewModel(context) {
    var type: String = AppConstant.TYPE_AFFIRMATION
    var keyword:String=""
    val favResponse by lazy { MutableSharedFlow<ResponseResult<ResponseWrapper>>() }
    val markFavResponse by lazy { MutableSharedFlow<ResponseResult<ResponseWrapper>>() }

    fun getFavouriteList() {
        viewModelScope.launch(Dispatchers.IO) {
            isLoading.emit(true)
            map.clear()
            map[ApiConstant.TYPE] = type
            map[ApiConstant.KEYWORD] = keyword
            val response = getResult({ apiService.getFavList(map) }, ApiEndPoint.GET_FAVOURITE_LIST)
            favResponse.emit(response)
            isLoading.emit(false)
        }
    }

    fun markFav() {
        viewModelScope.launch(Dispatchers.IO) {
            isLoading.emit(true)
            val response =
                getResult({ apiService.markFavourite(map) }, ApiEndPoint.MARK_FAVOURITE)
            markFavResponse.emit(response)
            isLoading.emit(false)
        }
    }
    override fun retry(type: String) {

    }
}